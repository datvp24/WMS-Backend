...
//            if (gi.Status == GIStatus.Complete)
//                throw new Exception("Completed GoodsIssue cannot be cancelled");

//            if (gi.Status == GIStatus.Rejected)
...
//                if (so.Status != SOStatus.Complete) throw new Exception("Only APPROVED SalesOrder can create GI");
...
//            if (dto.Status.HasValue) query = query.Where(x => x.Status == dto.Status);
...
//            if (gi == null) throw new Exception("GoodsIssue not found");
//            if (gi.Status != GIStatus.Pending) throw new Exception("Only PENDING GoodsIssue can be completed");
